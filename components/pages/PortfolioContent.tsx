...
export default function PortfolioContent() {
  const { t } = useLanguage();
  const p = t.pages.portfolio;
  const [active, setActive] = useState<Filter>("all");

  const filters: { value: Filter; label: string }[] = [
    { value: "all",         label: t.portfolio.filters.allProjects },
    { value: "residential", label: t.portfolio.filters.residential },
    { value: "commercial",  label: t.portfolio.filters.commercial },
    { value: "farm-nature", label: t.portfolio.filters.farmNature },
  ];

  const projectsWithText = projects.map((proj) => ({
    ...proj,
    ...t.portfolio.projects.find((i) => i.id === proj.id)!,
  }));

  const filtered =
    active === "all"
      ? projectsWithText
      : projectsWithText.filter((proj) => proj.category === active);
        ...
        <div className="max-w-[1200px] mx-auto px-5 md:px-8">
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20">
              <p className="font-raleway text-stone">{t.portfolio.noProjects}</p>
            </div>
          )}
        </div>
      </section>
